...
> **⚠️ Superseded — kept for the diagnosis, not the fix plan (noted 2026-07-27).**
>
> The four findings below are real and were the correct diagnosis at the time.
> **The remedy in §8 was not the one adopted.** That plan (an atomic
> `claim_shadow_profile()` that inserts a new profile with `id = new.id`,
> transfers every FK, then deletes the shadow) was superseded by
> `supabase/migrations/20260727110000_handle_new_user_shadow_claim.sql`, which
> takes the opposite approach — "S2", claim-in-place:
>
> signup **UPDATEs the shadow profile** (sets `user_id`, clears the shadow
> markers) and leaves its id untouched, so **there is nothing to transfer and
> nothing to cascade-delete**. Findings 1–4 dissolve rather than get patched.
> `profiles.id` is now independent of `auth.users.id`; `20260727100000` moved
> RLS and RPCs into profile-id space to make that safe.
>
> `claim_shadow_profile()` and `transfer_shadow_profile_references()` are **not
> part of the current architecture**. Read §§1–7 for how the lifecycle works
> and how it broke; ignore §8. The regression guard for the adopted model is
> `tests/e2e/auth/shadow-claim-with-data.spec.ts`.
