...
| Mockup                                  | Represents                     | Implementation                                                                                                                                               | Status                                                                                              | E2E                                                                                                               | Unit                                                                                                                                         |
| --------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Strummy - Teacher Dashboard.html        | Teacher home                   | `/dashboard` (role=teacher) → `TeacherDashboard`                                                                                                    | **mounted**                                                                                         | ✓ `dashboard/{sidebar,topbar,states}.spec.ts`, `teacher-full-journey.spec.ts`                                     | — (shell untested; only unrelated admin sub-widgets have tests)                                                                              |
...
| Strummy - Student Dashboard.html        | Student home                   | `/dashboard` (role=student) → `StudentDashboard`                                                                                                    | **mounted**                                                                                         | ✓ `dashboard/{sidebar,topbar,states}.spec.ts`, `student-full-journey.spec.ts`, `student-learning-journey.spec.ts` | —                                                                                                                                            |
| Strummy - Student Dashboard Mobile.html | Same, responsive               | same route, responsive                                                                                                                                       | **mounted**                                                                                         | ~ generic mobile dashboard case only                                                                              | —                                                                                                                                            |
| Strummy - Admin Dashboard.html          | Admin home                     | `/dashboard` (role=admin) → `AdminDashboard`                                                                                                        | **mounted**                                                                                         | ✓ `dashboard/{sidebar,topbar,states}.spec.ts`, `admin/{lockout-widget,system-logs,debug-dashboard}.spec.ts`       | ~ sub-widgets only (`AdminDashboardInsights`, `AIAssistantCard`, `EmailDraftGenerator`, `StudentProgressInsights`.test.tsx) — shell untested |
...
| Strummy - Student Detail -Healthy-.html | Student profile, healthy state | `/dashboard/users/[id]` → `StudentDetail` — one generic page, no status theming                                                                     | **mounted** (page exists; the "healthy" visual treatment itself isn't a distinct implemented state) | ✓ `teacher/users-management.spec.ts` (A6.2 renders profile, A6.4 edit/revert)                                     | ~ only the `.Repertoire` sub-tab is tested (`StudentDetailRepertoire...test`) — main shell untested                                 |
...
| Mockup                              | Represents            | Implementation                                                                                                                | Status                                                                                            | E2E                                                                    | Unit                                   |
| ----------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | -------------------------------------- |
| Strummy - Lesson List.html          | Teacher lesson list   | `/dashboard/lessons` → `LessonsList`                                                                                 | **mounted**                                                                                       | ✓ `teacher/lessons-crud.spec.ts`, `teacher/lesson-song-status.spec.ts` | —                                      |
...
| Strummy - Lesson Detail.html        | Lesson detail         | `/dashboard/lessons/[id]` → `LessonDetail`                                                                           | **mounted**                                                                                       | ✓ `lesson-song-status.spec.ts`, `lessons-crud.spec.ts`                 | —                                      |
...
| Mockup                                   | Represents               | Implementation                                                                                                      | Status                                                                                                                                                                                                                                                                          | E2E                                                                                    | Unit                                                                                                                           |
| ---------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Strummy - Song Detail.html               | Song detail              | `/dashboard/songs/[id]` → `SongDetail`                                                                     | **mounted**                                                                                                                                                                                                                                                                     | ✓ `teacher/songs-crud.spec.ts`, `student/songs-read.spec.ts`                           | ~ only `SongHero` sub-component tested                                                                                |
| Strummy - Song Form A.html               | Song create/edit, dir. A | `/dashboard/songs/new`, `/dashboard/songs/[id]/edit` → `SongForm` (create), `SongEditForm` (edit) | **mounted, redesigned 2026-07-23** (A/B were two design directions; one shipped) — both create and edit now use the same sectioned two-column + live-preview layout as the other dedicated forms, brought in line when that pattern shipped for Lesson/Student/Assignment forms | ✓ `songs-crud.spec.ts` (full create → view → edit → search → delete lifecycle)         | ✓ `SongForm.test.tsx`, `SongEditForm.test.tsx` (new 2026-07-23 — 3 cases: pre-fill, live preview, hidden id) |
...
| Strummy - Fretboard Explorer.html        | Chord/scale visualizer   | `/dashboard/fretboard` → `components/fretboard/`                                                          | **nav-hidden** (URL-reachable, hidden under "Tools")                                                                                                                                                                                                                            | ✓ `teacher/fretboard.spec.ts` — 10 cases (key/scale/chord/interval toggles, URL state) | ~ `fretboard.helpers.test.ts` (pure logic only, no component render test)                                                      |
...
| Mockup                              | Represents              | Implementation                                          | Status      | E2E                                                        | Unit                                                                                                                                                                        |
| ----------------------------------- | ----------------------- | ------------------------------------------------------- | ----------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Strummy - Assignments Teacher.html  | Teacher assign panel    | `/dashboard/assignments`, `/new`, `/[id]`, `/[id]/edit` | **mounted** | ✓ `assignments-crud.spec.ts`, `assignment-history.spec.ts` | ✓ `AssignmentsList.test.tsx`, `AssignmentDetail.test.tsx`, `AssignmentCreate.test.tsx` (updated 2026-07-23 for general fields/validation/submit) |
...
| Strummy - Notifications.html        | Notification inbox      | `/dashboard/notifications`                              | **mounted** | ✓ `notifications/inbox.spec.ts`                            | ✓ `Notifications.List.test.tsx`                                                                                                                                    |
...
| Strummy - Assignment Form (Standalone).html | Dedicated full-page assignment create/edit | `/dashboard/assignments/new`, `/[id]/edit` → `AssignmentCreate` (split into `.Fields`, `.Preview`, `useAssignmentFormSubmit`)                                    | **mounted, redesigned 2026-07-23** — sectioned layout + live preview shipped; single-student select kept (vs. mockup's multi-student chips — a true multi-assign would be a behavior change, not just layout, so deliberately out of scope) — no "submission type" selector, but has checklist + chord-drill editors the mockup doesn't | ✓ `teacher/assignments-crud.spec.ts`, `assignment-templates.spec.ts` (pre-existing)                                                                                                               | ✓ `AssignmentCreate.test.tsx` (broadened 2026-07-23 — was AI-wiring-only; added 5 cases for general fields/validation/submit)                                      |

## Settings (01)

| Mockup                               | Represents               | Implementation                                                                                   | Status                                              | E2E                                                                                                                                         | Unit                                                                   |
| ------------------------------------ | ------------------------ | ------------------------------------------------------------------------------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Strummy - Settings.html              | Settings home            | `/dashboard/settings` → `Settings` — one page, not tabbed                               | **mounted**                                         | ✓ `settings/api-keys.spec.ts`, `settings/avatar-upload.spec.ts`, `notifications/prefs.spec.ts`                                              | ~ `NotificationPreferences.test.tsx` sub-section only — shell untested |
...
they undercount current unit coverage significantly (e.g. every dashboard shell, `SongDetail`,
`Settings`, `Fretboard`, `ChordQuiz`, and `StudentDetail` gained tests in
...
component-level test** (21%) — mostly forms and list sub-parts (`SignInForm`, `SongForm`,
`Notifications.List`, `skeleton`). **12 more have partial coverage** (36%) — either a
...
(`TeacherDashboard`, `StudentDashboard`, `AdminDashboard`) and every
Lesson surface.
...
first has the thinnest coverage of the set) and the **dashboard shells** (`TeacherDashboard`
/ `StudentDashboard` / `AdminDashboard`) — all E2E-covered for role/nav wiring but
