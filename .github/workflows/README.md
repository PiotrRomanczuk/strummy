...
Two live workflows. **The `.yml` files' inline comments are the source of
truth** — they carry the measured timings, cost rationale, and past incident
notes. This README is only a map.

## `ci.yml` — quality gates + release (hosted runner)

Triggers: push to `main`, PRs targeting `main`. One job, one runner, one
`npm ci` — deliberately NO fan-out (see the cost note at the top of the file;
a previous pipeline ran up a $200+ Actions bill).

- **quality-gates**: lint + typecheck (concurrent), unit tests
  (PRs: `--changedSince=origin/main` affected-only; `main`: full suite +
  coverage), integration tests, Next.js build.
  Incremental caches keep warm runs ~2 min: `node_modules` (lockfile-keyed),
  `.eslintcache` + `tsconfig.tsbuildinfo`, and `.next/cache` (Turbopack
  filesystem cache — requires `experimental.turbopackFileSystemCacheForBuild`
  in `next.config.ts`).
- **release** (push to `main` only): cuts a git tag + GitHub Release from the
  merged PR — branch prefix decides the bump (`feature/` → minor, else patch),
  `version:major|minor|patch` labels override. PR body becomes the release
  notes. No version-bump commits; `package.json`'s version stays frozen.
...
- The self-hosted runner executes repo code on a home machine — fork PRs must
  never be allowed to run workflows. Review Actions settings whenever repo
  visibility changes.
- Deploys are Vercel Git-integration, not Actions (`vercel.json` gates which
  builds run).
- Dependabot: `../dependabot.yml` (weekly).
