...
const summarise = (lesson: ContinuityLesson, t: (key: string) => string): string => {
  ...
  return t('untitledLesson');
};

const ContinuityEntry = async ({
  lesson,
  isLast,
}: {
  lesson: ContinuityLesson;
  isLast: boolean;
}) => {
...
export const LessonContinuityCard = async ({
  lessons,
    ...
    <Card>
      ...
      <div style={{ padding: '10px 24px 18px' }}>
        {lessons.length === 0 ? (
          ...
          >
            {t('noPreviousLessons', { name: counterpartFirstName })}
          </div>
        ) : (
