...
export const HeroFloatChord = async () => {
  const t = await getTranslations('Landing.heroFloats');
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 3 }}>{t('chordCardTag')}</div>
      </div>
    </div>
  );
};
...
export const HeroFloatActivity = async () => {
  const t = await getTranslations('Landing.heroFloats');
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, lineHeight: 1.25 }}>
        ...
        <span style={{ color: 'var(--success)', fontWeight: 500 }}>{t('masteredLabel')}</span>{' '}
