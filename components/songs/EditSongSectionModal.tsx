...
export const EditSongSectionModal = ({ songId, section, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      section_number: formData.get('section_number')
        ? Number(formData.get('section_number'))
        : null,
  ...
  return (
    ...
    >
      <div
        data-testid="song-section-modal"
        style={{
      ...
      >
        ...
        >
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <input
              name="section_type"
              defaultValue={section?.section_type ?? 'verse'}
              required
              style={{ padding: 8, borderRadius: 4, border: '1px solid var(--rule)' }}
            />
          </label>
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
              ...
              <input
                name="order_position"
                type="number"
                defaultValue={section?.order_position ?? 1}
                required
                style={{ padding: 8, borderRadius: 4, border: '1px solid var(--rule)' }}
              />
            </label>
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <textarea
              name="chords"
              defaultValue={section?.chords?.join(' ') ?? ''}
              style={{
                padding: 8,
                borderRadius: 4,
