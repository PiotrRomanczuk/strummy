...
export const TeacherNoteCard = ({ note }: Props) => {
  ...
  return (
    <Card>
      <CardHeader eyebrow={t('detailNoteEyebrow')} title={t('detailNoteTitle')} />
      {!note ? (
        <Empty>{t('detailNoteEmpty')}</Empty>
      ) : (
        <div style={{ padding: '18px 24px 20px' }}>
          ...
          >
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-4)' }}>
              {t('detailNoteAfterLabel', {
                lessonTitle: note.lessonTitle ?? t('detailNoteLessonFallback'),
                date: formatDate(note.scheduledAt),
              })}
            </div>
            <Link
