...
export function LessonDetailSongs({ lessonId, songs, canEdit }: LessonDetailSongsProps) {
  ...
  const handleStatusChange = (songId: string, status: string) => {
    ...
    startTransition(async () => {
      await updateLessonSongStatus(
        lessonId,
        songId,
        status as 'to_learn' | 'started' | 'remembered' | 'with_author' | 'mastered'
      );
    });
  };
  ...
  return (
    <DetailCard
      ...
      title={
        <>
          Songs <span className="text-muted-foreground text-sm font-normal">· {songs.length}</span>
        </>
      }
      action={
        canEdit ? (
          <Button variant="outline" size="sm">
            <Plus className="h-3 w-3" /> Add song
          </Button>
        ) : undefined
      }
    >
  ...
  return (
    <div className="bg-card border border-border rounded-[10px] shadow-sm overflow-hidden">
      <div className="px-6 pt-5 pb-3 flex items-end justify-between">
        <div>
          <div className="font-mono text-[10px] text-muted-foreground uppercase tracking-[.14em] font-medium">
            {eyebrow}
          </div>
