...
export function SongDetailMobile({ song, isTeacher, onDelete }: SongDetailV2Props) {
  ...
  return (
    ...
    >
        ...
        <div className="w-full h-[200px] bg-card rounded-xl flex items-center justify-center relative overflow-hidden">
          ...
          {song.level && (
            <div className="absolute bottom-4 left-4">
              <span
                className={cn(
                  'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ring-1',
                  LEVEL_COLORS[song.level.toLowerCase()] ??
      ...
      {tabs.length > 1 && (
        <div
          className="flex border-b border-border/30 -mx-4 px-4 mt-4"
          role="tablist"
        ...
        >
          {tabs.map((tab) => (
            <button
              ...
              className={cn(
                'flex-1 py-3 text-sm font-bold text-center transition-colors min-h-[44px] relative',
                activeTab === tab.key
                  ? 'text-primary'
