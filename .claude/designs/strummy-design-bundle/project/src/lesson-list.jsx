// Lesson List — desktop table + mobile cards.
// Props: role, onOpen(lessonId), onCreate()

const LessonList = ({ role = 'teacher', onOpen, onCreate, onStart }) => {
  const [statusFilter, setStatusFilter] = React.useState(new Set(['scheduled','in_progress','completed','cancelled']));
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', minHeight:0 }}>
      ...
      <div style={{ padding:'28px 32px 20px', background:'var(--ivory)' }}>
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:18 }}>
          <div>
            <div style={{ color:'var(--ink-4)', fontSize:11, textTransform:'uppercase', letterSpacing:'.16em', fontFamily:'var(--mono)' }}>
              {role === 'student' ? 'Your lessons' : role === 'admin' ? 'All studio lessons' : 'Teaching'}
            </div>
            <h1 style={{ margin:'4px 0 0', fontFamily:'var(--serif)', fontWeight:400, fontSize:34, letterSpacing:'-0.02em' }}>
              Lessons
            </h1>
            <div style={{ color:'var(--ink-3)', fontSize:13, marginTop:6 }}>
              {rows.length} {rows.length === 1 ? 'lesson' : 'lessons'} · sorted by {sort === 'newest' ? 'newest first' : 'oldest first'}
            </div>
          </div>
          {canCreate && (
        ...
        <div style={{
          ...
          <span style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase',
                         letterSpacing:'.12em', fontFamily:'var(--mono)', marginRight:4 }}>Status</span>
          {Object.entries(LESSON_STATUS).map(([k, s]) => (
            <FilterChip key={k}
          ...
          <span style={{ fontSize:11, color:'var(--ink-4)', textTransform:'uppercase',
                         letterSpacing:'.12em', fontFamily:'var(--mono)', marginRight:4 }}>Year</span>
          <select value={yearFilter} onChange={e=>setYearFilter(e.target.value)} style={selectSm}>
      ...
      <div style={{ flex:1, overflow:'auto', padding:'0 32px 40px' }}>
        <div style={{
          background:'var(--card)', border:'1px solid var(--rule)', borderRadius:10, overflow:'hidden',
        }}>
          {/* header row */}
          <div style={{
            display:'grid',
            gridTemplateColumns: `88px 2.3fr ${showStudent ? '1.3fr ':''}${showTeacher ? '1.3fr ':''}1fr 110px 120px 60px`,
          ...
          {rows.map((l, idx) => {
            ...
            return (
              <div key={l.id} onClick={() => onOpen && onOpen(l.id)}
                style={{
                  display:'grid',
                  gridTemplateColumns: `88px 2.3fr ${showStudent ? '1.3fr ':''}${showTeacher ? '1.3fr ':''}1fr 110px 120px 60px`,
              ...
              >
                ...
                <div style={{ minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <span style={{
                      fontFamily:'var(--mono)', fontSize:10, color:'var(--ink-4)',
                      padding:'2px 6px', background:'var(--rule-2)', borderRadius:4,
                    }}>#{l.number}</span>
                    <span style={{
                  ...
                  {l.notes && (
                    <div style={{
                      color:'var(--ink-4)', fontSize:12, marginTop:4,
                      overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap',
                    }}>
                      {l.notes}
                    </div>
                ...
                {showTeacher && (
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <div style={{
                      width:24, height:24, borderRadius:'50%', background:l.teacher.color,
                      color:'#fff', display:'grid', placeItems:'center', fontSize:10, fontWeight:600,
                    }}>{l.teacher.avatar}</div>
                  ...
                  {l.songs.length > 0 && (
                    <span style={{ display:'inline-flex', gap:2, marginLeft:2 }}>
                      {l.songs.slice(0,4).map((s, i) => {
                        ...
                        return <span key={i} style={{ width:4, height:4, borderRadius:'50%', background:pc }} />;
                      })}
                    </span>
                  )}
                </div>
          ...
          {rows.length === 0 && (
            <div style={{ padding:'60px 20px', textAlign:'center', color:'var(--ink-4)' }}>
              <div style={{ fontFamily:'var(--serif)', fontSize:18, fontStyle:'italic', marginBottom:6 }}>
                Nothing here yet.
              </div>
...
const LessonListMobile = ({ role='teacher', onOpen, onCreate }) => {
  const [activeFilter, setActiveFilter] = React.useState('all');
  const rows = React.useMemo(() => {
    const r = [...LESSONS].sort((a,b) => new Date(b.scheduledAt) - new Date(a.scheduledAt));
    if (activeFilter === 'all') return r;
    return r.filter(l => l.status === activeFilter);
  }, [activeFilter]);
    ...
    <div style={{
      ...
      <div style={{ padding:'4px 20px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
          <h1 style={{ margin:0, fontFamily:'var(--serif)', fontWeight:400, fontSize:26, letterSpacing:'-0.02em' }}>
            Lessons
          </h1>
          {role !== 'student' && (
            <button onClick={onCreate} style={{
              ...
              <Icon d={LI.plusSmall} size={14} stroke="var(--paper)" />
            </button>
          )}
        </div>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={k} onClick={() => setActiveFilter(k)} style={{
            padding:'6px 12px',
            borderRadius: 999,
            border: activeFilter === k ? '1px solid var(--ink)' : '1px solid var(--rule)',
            background: activeFilter === k ? 'var(--ink)' : 'var(--card)',
            color: activeFilter === k ? 'var(--paper)' : 'var(--ink-3)',
            fontSize:12, flex:'0 0 auto', cursor:'pointer',
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'0 16px 24px' }}>
        {groups.map(([date, items], gi) => {
          const d = formatLessonDate(date + 'T00:00:00');
          return (
            <div key={date} style={{ marginBottom: 14 }}>
              <div style={{
                ...
                color:'var(--ink-4)', textTransform:'uppercase', letterSpacing:'.14em',
              }}>
                <span style={{ color:'var(--gold-2)', fontWeight:500 }}>{d.mon} {d.day}</span>
                <span>·</span>
                <span>{d.wday}, {d.year}</span>
                <span style={{ flex:1, height:1, background:'var(--rule)', marginLeft:6 }} />
              </div>
              {items.map((l) => (
                <div key={l.id} onClick={() => onOpen && onOpen(l.id)} style={{
                  background:'var(--card)',
                  border:'1px solid var(--rule)',
                  borderRadius: 10,
                  ...
                  {showStudent && (
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      ...
                      <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--ink-3)' }}>
                        {l.songs.length} {l.songs.length === 1 ? 'song' : 'songs'}
                      </span>
                      {l.songs.length > 0 && (
                        <span style={{ display:'inline-flex', gap:3, marginLeft:'auto' }}>
                          {l.songs.slice(0,5).map((s, i) => {
                            const pc = SONG_STATUS[s.progress].color;
                            return <span key={i} style={{ width:5, height:5, borderRadius:'50%', background:pc }} />;
                          })}
                        </span>
                      )}
                    </div>
...
// ── button / select styles reused across lesson views
const btnPrimary = {
...
const btnSecondary = {
  ...
  border:'1px solid var(--rule)', background:'var(--card)',
  color:'var(--ink), var(--ink-2)', fontSize:13, cursor:'pointer',
  display:'inline-flex', alignItems:'center', gap:6,
};
const selectSm = {
  padding:'5px 8px', borderRadius:6,
  border:'1px solid var(--rule)', background:'var(--card)',
  color:'var(--ink-2)', fontSize:12, fontFamily:'var(--sans)',
  cursor:'pointer',
};

Object.assign(window, {
  LessonList, LessonListMobile,
  btnPrimary, btnGhost, btnSecondary, selectSm,
});
